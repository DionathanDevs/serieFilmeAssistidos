...
verifyExceptions();
// chamando a função para verificar se a requisição foi feita via formulário (POST), se true não foi via POST, se false foi.
if(notForm()){
    header('location:loginweb.php?ms=0'); // redireciona para a mesma página, porém com o alerta de erro de requisição sem ser via post
    exit;
}

// chamando a função para verificar se os campos estão vazios se true é vazio, se false não está vazio
if(loginIsEmpty()){
    header('location:loginweb.php?ms=4'); // redireciona para a mesma página, porém com o alerta de erro que esta vazio os campos
    exit;
}
...
if(!mysqli_stmt_execute($stmt)){
    header('location:loginweb.php?ms=5');
    exit;
}
...
// armazena na váriavel as linhas afetadas na consulta, ou seja se não tiver nenhuma linha a consulta não encontrou nenhum resultado
$rows = mysqli_stmt_num_rows($stmt);

// condicao para verificar se houve linhas afetadas
if($rows !== 1){
    header('location:loginweb.php?ms=7');
    exit;
}
